...
import CreatePagesDialog from "./CreatePagesDialog";
...
const  Pages = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get("https://aihika.in/internal-software/public/api/v1/pages", {
        headers: {
          Authorization: `Bearer ${login.token}`,
        },
      });

      const transformedData = response?.data?.data.map(pages => ({
  ...
  return (
    <Box sx={{ height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <CreatePagesDialog open={open} handleClose={handleClose} />
    </Box>
  );
};
