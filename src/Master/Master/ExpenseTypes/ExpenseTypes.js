...
import CreateExpenseTypesDialog from "./CreateExpenseTypesDialog";
...
const ExpenseTypes = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get("https://aihika.in/internal-software/public/api/v1/expense-types", {
        headers: {
          Authorization: `Bearer ${login.token}`,
        },
      });

      const transformedData = response?.data?.data.map(expense => ({
  ...
  return (
    <Box sx={{ height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <CreateExpenseTypesDialog open={open} handleClose={handleClose} />
    </Box>
  );
};
